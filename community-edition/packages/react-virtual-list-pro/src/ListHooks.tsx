...
const ResizeObserver = (globalObject as any).ResizeObserver || RO;
...
type TypeProps = {
  ...
  count: number;
  renderScroller?: (props: any) => void;
  rowHeightManager?: any;
  rowHeight: number;
  renderScrollerSpacer?: (props: any) => void;
  rtl: boolean;
  renderView?: (viewProps: any) => void;
  minRowHeight?: number;
  virtualized?: boolean;
  extraRows?: number;
  ...
  scrollOneDirectionOnly?: boolean;
  onScrollStop?: (...args: any) => void;
  applyScrollLeft?: (scrollLeft: number, domNode: ReactNode) => void;
  useTransformPosition?: boolean;
  naturalRowHeight?: boolean;
  onRenderRangeChange?: (startIndex: number, endIndex: number) => void;
  measureSize?: (node: ReactNode) => { width: number; height: number };
  scrollProps?: TypeScollProps;
  onResize?: (size: { width: number; height: number }) => void;
  renderRow?: (props: TypeProps) => void;
  pureRows?: boolean;
  rowContain?: boolean;
  useTransformRowPosition?: boolean;
  handleRowKeyDown?: (index: number, event: KeyboardEvent) => void;
  getRowFocusableElements?: (index: number, rowNode: ReactNode) => void;
  shouldPreventDefaultTabKeyOnRow?:
    | boolean
    | ((index: number, event: KeyboardEvent) => boolean);
  ...
  ResizeObserver?: any;
  contain?: string;
  shouldAllowScrollbars?: () => boolean;
  rafOnResize?: boolean;
};

const InovuaVirtualList = forwardRef((props: TypeProps, ref: any) => {
  const forceUpdate = useForceUpdate();

  const stickyRowsContainerRef = useRef<any>(null);
  const scrollContainerRef = useRef<any>();
  const rowRef = useRef<any>(null);
  const containerNode = useRef(null);
  const strictVisibleCountRef = useRef<number>(0);
  const visibleCountRef = useRef<number>(0);
  const rowOffsetsRef = useRef<number[] | null>([]);
  const rowHeightsRef = useRef<number[] | null>([]);
  const scrollTopPosRef = useRef<number>(0);
  const scrollLeftPosRef = useRef<number>(0);
  const rowsRef = useRef<Row[] | null>([]);
  const mappingRef = useRef<any>({});
  const scrollingRef = useRef<boolean>(false);
  const disableScrollPropNameRef = useRef<'overflowX' | 'overflowY'>();
  const disableScrollOtherPropNameRef = useRef<'overflowX' | 'overflowY'>();
  const disableScrollPropValueRef = useRef<any>();
  const oldRowIndexesRef = useRef<number[]>([]);
  const forceScrollTopRef = useRef<number | undefined>(0);
  const scrollTopRef = useRef<number>(0);
  const scrollLeftRef = useRef<number>(0);
  const prevScrollTopPosRef = useRef<number>(0);
  const prevScrollLeftPosRef = useRef<number>(0);
  const sizeRef = useRef<{ height: number; width: number }>({
    height: 0,
    width: 0,
  });
  const prevStartRowIndexRef = useRef<number>(0);
  const rowCoveredByRef = useRef<any>({});
  const updateRafHandleRef = useRef<any>();
  const mountedRef = useRef<boolean>(false);
  const unmountedRef = useRef<boolean>(false);
  const rowSpansRef = useRef<any>({});
  const currentStickyRowsRef = useRef<any>();
  ...
  useEffect(() => {
    let prevScrollTopPos: number | undefined;
    let prevScrollLeftPos: number | undefined;
  ...
  const getVisibleCount = useCallback(
    (theProps: TypeProps = props): number => {
      const {
        virtualized,
        enableRowSpan,
        extraRows: extraRowsProps,
      } = theProps;
  ...
  const getMaxRenderCount = useCallback(
    (theProps: TypeProps = props) => {
  ...
  const initSizes = useCallback(
    (theProps: TypeProps = props) => {
      ...
      const rowHeights: any = [...Array(count)].map(() => {
  ...
  const getTotalRowHeight = useCallback((theProps: TypeProps = props) => {
  ...
  const renderScrollerSpacerOnNaturalRowHeight = useCallback(
    (spacerProps: any) => {
   ...
   */
  const setHeightForRows = useCallback(
    (indexes: number[], heights: number[]) => {
  ...
  const renderSizer = useCallback(
    (scrollHeight: number) => {
      ...
      if (showEmptyRows) {
        minHeight = Math.max(
          minHeight,
          strictVisibleCountRef.current * rowHeightManager.getDefaultRowHeight()
        );
      }
  ...
  const onScrollbarsChange = useCallback(
    ({ vertical, horizontal }: { vertical: boolean; horizontal: boolean }) => {
  ...
  const onRowUnmount = useCallback((row: Row) => {
  ...
  const rowRefFn = useCallback((r: Row) => {
    if (!r) {
      return;
    }
  ...
  const onScrollStop = useCallback(
    (...args: any) => {
      ...
      if (props.scrollOneDirectionOnly) {
        ...
        scrollerNode.styprops.scrollOneDirectionOnlyle[
          disableScrollPropNameRef.current!
        ] = disableScrollPropValueRef.current;
  ...
  const forEachRow = useCallback(
    (fn: (row: Row, i?: number) => void, onlyVisible: boolean = true) => {
      const rows: any = rowsRef.current;
      const visibleCount: number = getVisibleCount();
      let i = -1;
  ...
  const sortRows = useCallback((rows: Row[]) => {
    return rows.slice().sort((row1, row2) => row1.getIndex() - row2.getIndex());
  }, []);

  const getRowAt = useCallback((index: number) => {
  ...
  const setRowIndex = useCallback(
    (row: Row, index: number, callback?: () => void) => {
  ...
  const getVisibleRangeFn = useCallback((args: any): {
    start: number;
    end: number;
  } => {
  ...
  const applyScrollStyle = useCallback(
    (
      {
        scrollTop,
        scrollLeft,
      ...
      const size: { height: number; width: number } = sizeRef.current;
      ...
      const updateScroll = (top = scrollTop!) => {
        const containerNode: any = getContainerNode();
        const parentNodeStyle = containerNode && containerNode.parentNode.style;
        containerNode &&
          containerNode.parentNode.classList.add(
  ...
  const setupRowHeightManager = useCallback(
  ...
  const onIndex = useCallback(() => {
    ...
    const rows: Row[] = rowsRef.current!;
    rows &&
  ...
  const refreshLayout = useCallback(
    (config: { reorder?: boolean; force?: boolean }) => {
      const defaults = {
        force: true,
        reorder: true,
      };
  ...
  const updateVisibleCount = useCallback(
    (height: number, theProps?: TypeProps) => {
  ...
  const getCleanupRows = useCallback(
    (theProps: TypeProps = props) => {
      const indexes = [];
      const rows = rowsRef.current;
  ...
  const setRowRowSpan = useCallback((rowIndex: number, rowSpan: number) => {
    if (rowSpan === 1) {
      return;
    }
  ...
  const onRowKeyDown = useCallback(
    (index: number, event: KeyboardEvent) => {
      ...
      if (
        typeof props.shouldPreventDefaultTabKeyOnRow !== 'function' ||
  ...
  const getRowVisibilityInfo = useCallback(
    (
      index: number,
      offset: number
    ): {
  ...
  const focusRow = useCallback(
    (index: number, dir: -1 | 1, callback?: () => void) => {
      ...
      scrollToIndex(index, { direction: dir == 1 ? 'bottom' : 'top' }, () => {
        ...
        const elements: any = props.getRowFocusableElements
  ...
  const isRowRendered = useCallback((index: number) => {
  ...
  const updateStickyRows = useCallback(
    (
      scrollTop: number = scrollTopRef.current,
      ...
      const currentStickyRowsMap: boolean[] = [];
      ...
      rowsPerScales.forEach(rows => {
        ...
        if (stickyRow) {
          ...
          const nextRow: any = allRows[stickyRow.indexInAllRows + 1];
      ...
      enteringRows = enteringRows.filter(
        (row: any) =>
          !currentStickyRowsMap[row.index] && row.index > maxStickyRowIndex
      );

      const enteringRow = enteringRows[0];

      if (
        JSON.stringify(currentStickyRowsRef.current) ===
  ...
  const getFirstVisibleRowIndexForSticky = useCallback(
    (scrollTop: number = scrollTopPosRef.current): number => {
      const { rowHeightManager }: any = props;

      const stickyHeight = currentStickyRowsRef.current
        ? currentStickyRowsRef.current.reduce(
            (_: any, row: TypeStickyRowInfo) => {
  ...
  const setStickyRows = useCallback(
    (
      currentStickyRows: TypeStickyRowInfo[] = currentStickyRowsRef.current,
      ...
      stickyRowsContainerRef.current.setStickyRows(
  ...
  const getRenderedIndexes = useCallback(() => {
    return Object.keys(mappingRef.current).map((k: any) => k * 1);
  }, []);

  const onRowFocus = useCallback((_index: number, _event: Event) => {}, []);
  ...
  const className = join(
    props.className,
    BASE_CLASS_NAME,
    props.theme && `${BASE_CLASS_NAME}--theme-${props.theme}`,
  ...
  return (
    <Factory
      ...
      ResizeObserver={
        props.ResizeObserver !== undefined
          ? props.ResizeObserver
          : ResizeObserver
      }
      extraChildren={stickyRowsContainer}
      useTransformToScroll={props.useTransformPosition}
      ...
      applyScrollStyle={applyScrollStyle}
      onResize={onResize}
...
const propTypes = {
  ...
  rowHeightManager: (props: any, propName: string) => {
...
(InovuaVirtualList as any).propTypes = propTypes;
